...
for name, rotations in PIECES.items():
    PIECES[name] = [
        [[int(i) for i in p] for p in r.splitlines()] for r in rotations
    ]
...
#		print("Position testée valide: x=%s, y=%s" % (x, y))
    ...
    def first(self):
        ...
        self.current, self.next, self.perdu = None, self.get_piece(), False

    def next(self):
        print("Piece suivante")
        self.current, self.next = self.next, self.get_piece()
        self.pieces += 1
